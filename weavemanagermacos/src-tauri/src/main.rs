...
fn fetch_minecraft_instances(app_state: State<AppState>) -> Vec<MinecraftInstance> {
    ...
    let instances: Vec<_> = system.processes().values()
        .filter_map(|proc| {
            // println!("Processing process with PID: {}", proc.pid());

            if !matches!(proc.exe().file_name().and_then(OsStr::to_str), Some("javaw.exe" | "java")) {
                println!("Skipping process due to incorrect executable: {:?}", proc.exe());
                return None;
            }

            // skip cuz its fucking macos 
            // if !proc.cmd().iter().any(|arg| arg.contains(".minecraft")) {
            //     println!("Skipping process without .minecraft in command: {:?}", proc.cmd());
            //     return None;
            // }

            let client_type = if proc.cmd().iter().any(|arg| arg.contains("lunar")) {
                // println!("Detected Lunar Client");
                ClientType::LunarClient
            } else if proc.cmd().iter().any(|arg| arg.contains("minecraftforge")) {
                // println!("Detected Forge");
                ClientType::Forge
            } else {
                // println!("Detected Vanilla");
                ClientType::Vanilla
            };

            let weave_attached = if proc.cmd().iter().any(|arg| arg.contains("loader.jar") && arg.contains("-javaagent")) {
                // println!("Weave attached");
                true
            } else {
                // println!("Weave not attached");
                false
            };
    ...
    println!("Finished processing instances. Found {} instances.", instances.len());
    instances
}
